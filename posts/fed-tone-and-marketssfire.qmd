...
The table below reports the estimated coefficients on tone for each of the six event-level regressions, using HC3 robust standard errors. Stars indicate statistical significance: *** at 1%, ** at 5%, * at 10%.
...
Two results in this table stand out, and they tell different stories.

**Model 1 — tone level vs. 2-year yield change.** The coefficient is statistically significant at the 5% level, but the **sign is negative**, which is the opposite of what H1 predicts: more hawkish-coded statements are associated with same-day *falls* in the 2-year yield, not rises. This counter-intuitive finding deserves careful interpretation rather than being read as confirmation of H1.

**Model 4 — tone *change* vs. 2-year yield change.** When I instead measure tone as the change since the previous statement (a rough proxy for a communication surprise), the coefficient is **positive** (3.67) and marginally significant at the 10% level. This is in the direction H1 predicts: when the Fed becomes *more* hawkish than at its previous meeting, short-term yields tend to rise on the day of the statement. This is a more economically sensible result, because asset prices should respond to news rather than to levels that markets have already priced in.

The contrast between these two models is one of the most informative findings in the project, and is discussed further below. The remaining four regressions are statistically insignificant or only marginally so, and the R-squared values are uniformly low.

The negative sign on the *level* of tone (Model 1), combined with the positive sign on the *change* in tone (Model 4), points to a single coherent story:
...
2. **The change in tone is a better proxy for surprise.** When the statement is more hawkish than the previous one (Δtone > 0), the 2-year yield does tend to rise, in line with H1. The fact that this surprise-style measure produces the right sign, even though it is a crude proxy, is supportive evidence that FOMC communication does carry information for short-term rates.
3. **The tone score is dominated by the word "inflation".** Around two-thirds of all hawkish hits in the sample come from the single word "inflation". Combined with the fact that statements have got progressively shorter since 2022, this means that movements in normalised tone partly reflect how much of each statement is given over to discussing inflation, not whether the Fed sounds hawkish about it. A more sophisticated text model could distinguish "inflation has eased" from "inflation remains elevated", but a simple dictionary cannot.
...
The dictionary approach is transparent and easy to reproduce, but it cannot fully capture context. For example, the word "inflation" may appear in a hawkish context if the Fed says inflation remains elevated, but it may appear in a less hawkish context if the Fed says inflation has moderated. This means the tone score should be interpreted as a simplified proxy for communication tone rather than a complete measure of policy stance. In addition, the word "inflation" alone accounts for the majority of hawkish hits in this sample, so the tone score is in practice closer to "inflation density" than to a balanced hawkish-versus-dovish measure.

### Sparse dovish counts

Around half of the statements in the sample contain zero matches from the dovish dictionary. This reflects the fact that FOMC statements rarely use direct dovish vocabulary even when policy is being eased; the message is more often delivered through what the statement *omits* than through dovish words. This is a known weakness of small hand-built dictionaries and is one of the reasons the absolute tone level is hard to interpret.

### Market expectations

The analysis does not directly measure market expectations before each FOMC statement. This matters because asset prices should react mainly to unexpected information. If markets already expected hawkish language, the actual market reaction may be small even if the statement itself contains many hawkish terms. A more sophisticated event study would use intraday futures pricing of the policy rate (e.g., Fed funds futures or OIS) to construct a true monetary-policy surprise, which is beyond the scope of this project.

### Event-window problem

The project uses same-day market data. This is practical and reproducible, but it means the measured market reaction may include other information released on the same day, such as the policy rate decision, Summary of Economic Projections, press conference comments, or unrelated macroeconomic news. The standard event-study approach in the academic literature (e.g., Gürkaynak, Sack and Swanson, 2005) uses a 30-minute window around the release to isolate the announcement effect.

### Sample size and multiple testing

The final dataset contains 42 standard FOMC statements. This is enough for a small event-study project, but it limits statistical power. I also run six regressions, so a single significant result at the 5% level is roughly what one would expect by chance, even if there were no underlying relationship. The results should therefore be interpreted as suggestive evidence rather than strong causal proof.
...
The main contribution of the project is methodological: it shows how Python, web scraping, text processing, financial data, and regression analysis can be combined into a reproducible event-based study of central bank communication. The headline result has the "wrong" sign at the level of tone, but the right sign once tone is measured as a surprise — which is a useful demonstration of why dictionary measures of central bank communication need to be paired with a notion of market expectations to be properly interpreted. Future work could extend this with a true monetary-policy surprise series (e.g. from Fed funds futures), an intraday event window, or a context-aware text model that can distinguish "inflation remains elevated" from "inflation has eased".
